<%@page import="java.text.SimpleDateFormat"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %> 
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>

<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
<!-- 공통 CSS -->
<link rel="stylesheet"
	href="<c:url value="/resources/css/user/common/common.css"/>">

<!-- 페이지 CSS  -->
<link rel="stylesheet"
	href="<c:url value="/resources/css/user/store/products/products.css?v=<%=System.currentTimeMillis() %>"/>">


<!-- 라이브러리 -->
<script
	src="<c:url value="/resources/vendor/jquery/jquery-3.5.1.min.js"/>"></script>
<script src="<c:url value="/resources/vendor/fontawesome/js/all.js"/>"></script>
<script type="text/javascript"
	src="https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js"></script>
<script src="https://unpkg.com/swiper/swiper-bundle.min.js"></script>
<script
	src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>




</head>
<body>
	<% 
		/* 공통 Header */
	%>
	<jsp:include page="/WEB-INF/views/user/include/top-menu.jsp" />
	<% 
		/* 공통 Header */
	%>
	<header class="sticky-top">
		<jsp:include page="/WEB-INF/views/user/include/header.jsp" />
		<jsp:include page="/WEB-INF/views/user/include/side-nav.jsp" />
	</header>
	<% 
		/* 각 페이지의 특성! */
		
	%>
	<section>
	
		<div class="product_all">
			<div class="product_all2">
				<div class="product_all3">
					<div class="product_all4">
						<div class="product_all5">
							<div size="310" class="product_all6">
								<div class="product_all7">
									<div class="product_all8"></div>
								</div>
								<jsp:include page="/WEB-INF/views/user/store/common/store_header.jsp" />
								<%-- <div class="product_all9">
									<a class="sc-hZhUor dKChES" href="/store/782819/products"><img
										src="data:image/png;base64,"
										width="100" height="100" alt="상점 프로필 이미지"
										class="product_all10"></a>
									<div class="product_all11">${ storeVO.storeName }</div>
									<div class="product_all12">
										<img
											src="data:image/png;base64,"
											width="15" height="14" alt="작은 별점 0점 이미지"><img
											src="data:image/png;base64,"
											width="15" height="14" alt="작은 별점 0점 이미지"><img
											src="data:image/png;base64,"
											width="15" height="14" alt="작은 별점 0점 이미지"><img
											src="data:image/png;base64,"
											width="15" height="14" alt="작은 별점 0점 이미지"><img
											src="data:image/png;base64,"
											width="15" height="14" alt="작은 별점 0점 이미지">
									</div>
									<c:set var="storeBtn1" value="${user.storeNo}" />
									<c:set var="storeBtn2" value="${storeVO.storeNo }" />
										
										<c:if test="${storeBtn1 eq storeBtn2}">
											<div class="product_all13">
												<a class="product_all14 product_all14_1"
													href="${pageContext.request.contextPath}/shop/${storeVO.storeNo }/info.do">내 상점 관리</a>
													
												<a class="product_all14 product_all14_1"
													href="/products/manage">내 상품 관리</a>
											</div>
									</c:if>
								</div>
							</div>
						</div>

					
						<div class="product_all15">
							<div class="product_all16">
								<div class="product_all17">
									${ storeVO.storeName }
									<!-- <button class="product_name">상점명 수정</button> -->
								</div>
								
							</div>
							<div class="product_all20">
								<div class="product_all21">
									<img
										src="data:image/png;base64,"
										width="14" height="13" alt="상점오픈일 아이콘">&nbsp;&nbsp;상점오픈일
										<% 
											SimpleDateFormat nowDate = new SimpleDateFormat ( "yyyy-MM-dd");
											String nowDate1 = nowDate.format (System.currentTimeMillis());
										%>
										<fmt:parseDate value="${nowDate1 }" var="strNowDate" pattern="yyyy-MM-dd"/>
										<fmt:parseNumber value="${strNowDate.time / (1000*60*60*24)}" integerOnly="true" var="nowDate"></fmt:parseNumber>
										<fmt:parseDate value="${storeVO.userRegYmd}" var="userRegDate" pattern="yyyy-MM-dd"/>
										<fmt:parseNumber value="${userRegDate.time / (1000*60*60*24)}" integerOnly="true" var="regDate"></fmt:parseNumber>
										
									<div class="product_all22">${nowDate-regDate} 일</div>
								</div>
								<!-- <div class="product_all23">
									<img
										src="data:image/png;base64,"
										width="14" height="13" alt="상점방문수 아이콘">&nbsp;&nbsp;상점방문수
									<div class="product_all24">4명</div>
								</div> -->
								<div class="product_all25"><img
                                    src="data:image/png;base64,"
                                    width="14" height="13" alt="상품판매 아이콘">&nbsp;&nbsp;상품판매<div class="product_all26">
                                    0 회</div>
                            </div>
                            <!-- 
                            <div class="product_all27"><img
                                    src="data:image/png;base64,"
                                    width="14" height="13" alt="상퓸발송 아이콘">택배발송<div class="product_all28">
                                    0 회</div>
                            </div> -->
							</div>
							<div class="product_all29">
								${ storeVO.storeIntro }
							</div>
							<!-- <div class="product_all30">
								<button class="product_all31">소개글 수정</button>
							</div> -->
						</div>
 --%>

						<div class="product_all32">
							<div class="product_all33">
								<button class="product_all34">
									<img
										src="data:image/png;base64,"
										width="16" height="17" alt="닫기 버튼 아이콘">
								</button>
								<img
									src="data:image/png;base64,"
									width="82" height="82" alt="연락하기 프로필 이미지" class="product_all35">
								<div class="product_all36">
									<div class="product_all37">${ storeVO.storeName }</div>
								</div>

								<div class="product_all38">
									<!-- <div class="product_all39">
										<div class="product_all40">연락가능시간</div>
										<div class="product_all41">
											<span class="product_all42">24시간</span>
										</div>
									</div> -->

									<div class="product_all43">
										<div class="product_all44">상점별점후기</div>
										<div class="product_all45">
											<div class="product_all46">
												<img
													src="data:image/png;base64,"
													width="20" height="19" alt="큰 별점 0점 이미지">
												<img
													src="data:image/png;base64,"
													width="20" height="19" alt="큰 별점 0점 이미지">
												<img
													src="data:image/png;base64,"
													width="20" height="19" alt="큰 별점 0점 이미지">
												<img
													src="data:image/png;base64,"
													width="20" height="19" alt="큰 별점 0점 이미지">
												<img
													src="data:image/png;base64,"
													width="20" height="19" alt="큰 별점 0점 이미지">
											</div>
										</div>
									</div>
									<!-- <div class="product_all47">
										<div class="product_all48">상점연락처</div>
										<div class="product_all49">연락처 비공개</div>
									</div> -->
								</div>

								<!-- <div class="product_all50">
									<div class="product_all51">
										<span class="product_all52">안전한 거래를 위해 번개톡으로 연락해주세요</span>(번개톡
										&gt; 사기내역조회필수!)
									</div>
								</div> -->
								<!-- <div class="product_all53">
									<a class="product_all54"><img
										src="data:image/png;base64,"
										width="20" height="19" alt="번개톡 버튼 아이콘">번개톡</a><a
										class="product_all55"><img
										src="data:image/png;base64,"
										width="13" height="20" alt="연락처확인 버튼 아이콘">연락처확인</a>
								</div> -->
							</div>
						</div>
					</div>
				</div>

				<div class="sc-dCVVYJ dYImYF">
					<div class="sc-vBKru hEaZqn">
						<div class="product_all56">
							<a class="product_all57" href="${pageContext.request.contextPath}/shop/${storeVO.storeNo }/products.do">상품 
								<span class="product_all58">${fn:length(storeProductVO)}</span>
							</a> 
							<a class="product_all59" href="${pageContext.request.contextPath}/shop/${storeVO.storeNo }/comments.do">상점문의 
								<span class="product_all60">${fn:length(storeCommentVO)}</span>
							</a> 
							<a class="product_all61" href="${pageContext.request.contextPath}/shop/${storeVO.storeNo }/favorites.do">찜 
								<span class="product_all62">${fn:length(storeFavoriteVO)}</span>
							</a>
							<a class="product_all63" href="${pageContext.request.contextPath}/shop/${storeVO.storeNo }/reviews.do">상점후기 
								<span class="product_all64">${fn:length(storeReviewVO)}</span>
							</a>
							
						</div>
					</div>
					<div class="sc-kIXKos cxqMue">
						<div class="product_all69">
							<div class="product_all70">
								<div>
									상품 <span class="product_all71">${fn:length(storeProductVO)}</span>
								</div>
								<div class="product_all72"></div>
							</div>

							<div class="product_all78">
								<div class="product_all79">
									<div class="product_all80">
										<div class="product_all81">
											<div>전체</div>
											<div class="product_all82">${fn:length(storeProductVO)}개</div>
										</div>
										<!-- <div class="product_all83">
											<a class="product_all84">최신순</a><a class="product_all85">인기순</a><a
												class="product_all86">저가순</a><a class="product_all87">고가순</a>
										</div> -->
									</div>
								</div>


								<!------------------------- 상품 영역 ----------------------------->
								<div class="product_all88" id="result">
								<c:forEach var="storeProductVO" items="${storeProductVO}">
									<div class="product_all89">
										<a data-pid="128683129" class="product_all90"
											href="/products/128683129?ref=%EC%83%81%EC%A0%90%EC%A0%84%EC%B2%B4%EC%83%81%ED%92%88">
											<div class="product_all91">
												<img
													src="https://media.bunjang.co.kr/product/128683129_1_1594273655_w292.jpg"
													alt="상품 이미지">
											</div>
											<div class="product_all92">
												<div class="product_all93">${storeProductVO.productName }</div>
												<div class="product_all94">
													<div class="product_all95">${storeProductVO.productPrice }</div>
													<div class="product_all96">
														<span>${storeProductVO.productRegDt }</span>
													</div>
												</div>
											</div>
											<div class="product_all97">
												<img
													src="data:image/png;base64,"
													width="15" height="17" alt="위치 아이콘">${storeProductVO.productDealArea }
											</div>
										</a>
									</div>
									</c:forEach>
								</div>
							
								<div class="product_all98"></div>
							</div>
						</div>
					</div>
				</div>
			</div>

		</div>

	</section>
	<% 
		/* 공통 푸터 */
	%>
	<jsp:include page="/WEB-INF/views/user/include/footer.jsp" />
</body>
</html>